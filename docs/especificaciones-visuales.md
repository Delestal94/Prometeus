...
| # | Especificación | Prio |
|---|---|---|
| 21 | ~~Balanceo de carrocería exagerado en curvas y frenadas.~~ **[x] Hecho (2026-09-21), con alcance acotado.** `VehiclePresentation` inclina y hunde solo `BodyVisuals` (la carrocería exterior), no `CabinInterior`/`CargoBay` — inclinar los asientos también inclinaría cada `FirstPersonCamera` anclada ahí, sumando un tercer movimiento continuo encima de la sacudida y el head bob que ya tienen. Se nota para pasajeros mirando por la ventana, otros jugadores viendo pasar la furgoneta, y la futura cámara de desarrollo (#75). El peso "sentido" por el conductor queda como mejora futura, más riesgosa de probar sola. | **A** |
| 22 | ~~Asentamiento del paquete: rebote al apoyarlo.~~ **[x] Hecho (2026-09-21)** — nueva señal `EventBus.package_placed`, emitida por `place_at()`. `package_feedback.gd` responde con una curva de resorte amortiguada sobre la escala de la caja (se comprime y sobrepasa antes de asentarse), separada de la escala de Peso Creciente para que ambas se combinen en vez de pisarse (`_update_box_scale`). | **A** |
| 23 | ~~Sacudida del paquete proporcional al golpe.~~ **[x] Hecho (2026-09-21)** — reutiliza `package_damaged` (ya existía) para una sacudida propia que decae solo, en la malla real del paquete, para cualquier trampa (antes solo Ruidoso tenía sacudida, y era continua, no por golpe). Se combina de forma aditiva con el temblor de Ruidoso en vez de pisarlo. | **A** |
...
| 99 | ~~Reacción del paquete al input del jugador.~~ **[x] Ya estaba mayormente cubierto, revisado 2026-09-21.** Equilibrio ya se corrige físicamente de verdad (#25). Ruidoso ya se calma visible y audiblemente en cuanto sueltan `calm` (el temblor y el gemido ya siguen `_distress`, que baja apenas se calma). Peso Creciente ya se encoge al resolver la secuencia completa (mismo camino que el crecimiento, vía `package_integrity_changed`). **Lo que falta** — un pulso visible por cada flecha correcta de la secuencia, no solo al completarla entera — queda como pulido menor, no como hueco real. | **A** |
