...
func _run() -> void:
	...
	assert(manager.is_unlocked(&"growing_weight_trap"), "Una entrega desbloquea Peso creciente")
	manager.record_run(0, {"delivered": false})
	manager.record_run(0, {"delivered": true})
	assert(manager.is_unlocked(&"noisy_trap"), "Dos entregas y 100 puntos desbloquean Ruidoso")
	manager.record_run(0, {"delivered": true})
	assert(not manager.is_unlocked(&"liquid_trap"), "Líquido sigue bloqueado tras tres entregas")
	manager.record_run(0, {"delivered": true})
	assert(manager.is_unlocked(&"liquid_trap"), "Cuatro entregas y 250 puntos desbloquean Líquido")
	...
	assert(restored.successful_deliveries == 4, "Las entregas deben persistir")
